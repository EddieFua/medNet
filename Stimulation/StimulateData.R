...
StimulateData_update <- function(n_exposure, n_mediator, N, ba_m, ba_power, multi_rate, poisson_rate,
  ...
  Sigma_matrix_m <- 0.3^ba_result$dist
  ...
  mediator_my[,id[,2]] <- mediator_my[,id[,2]]+exposure_my[,id[,1]]
  low<-1
  high<-2
  ...
  Z <- cbind(matrix(rep(1,N)),exposure_my[,1:true_exposure])%*%beta + mediator_my%*%alpha
